...
typedef struct __attribute__((packed)) dgus_cmd_system_config {  //0x80
  ...
  // uint8_t screen_orientation_2:1; 
  uint8_t enble;        /**< Current screen orientation. 00=0, 01 = 90, 10=180, 11=270 */
} dgus_cmd_system_config;                /**< sys config*/

//0x5A000030

/**
 * @brief Layout of the rtc  command register
 * 
 * @return typedef struct 
 */
typedef struct __attribute__((packed)) rtc_cmd_register {
  uint8_t year;                     /**< Start playback of the Wav Id */
  uint8_t month;                   /**< Play back on the segment id */
  uint8_t day;                       /**< Volume level to play 0-0x80 */
  uint8_t week;    
  uint8_t hour;         
  uint8_t minute;         
  uint8_t second;      
...
/**
 * @brief write  the system configuration variable
 *
 * @param config
 * @return #DGUS_RETURN
 */
