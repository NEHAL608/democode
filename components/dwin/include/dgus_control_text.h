...
typedef struct __attribute__((packed)) dgus_control_text_display {
 ...
 /**
 * @brief clear from VAR address as text
...
DGUS_RETURN dgus_set_rolltext(uint16_t addr, char *text, uint8_t len);
 ...
 * @return Response such as #DGUS_TIMEOUT
 */
DGUS_RETURN dgus_set_text_padded(uint16_t addr, char *text, uint16_t len);
