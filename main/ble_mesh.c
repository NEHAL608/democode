...
#define ESP_BLE_MESH_VND_MODEL_OP_SEND      ESP_BLE_MESH_MODEL_OP_3(0x03, CID_ESP)
#define ESP_BLE_MESH_VND_MODEL_OP_STATUS    ESP_BLE_MESH_MODEL_OP_3(0x01, CID_ESP)
#define ESP_BLE_MESH_VENDOR_MODEL_OP_GET_TEMPERATURE       ESP_BLE_MESH_MODEL_OP_3(0x02, CID_ESP)
#define ESP_BLE_MESH_VENDOR_MODEL_OP_TEMPERATURE_STATUS    ESP_BLE_MESH_MODEL_OP_3(0x04, CID_ESP)

extern  DGUS_obj bluetoothButton;
static uint8_t dev_uuid[ESP_BLE_MESH_OCTET16_LEN];
uint16_t previous_cmd;
uint8_t previous_value;
bool ispaired =0;
uint16_t  ble_temprature,ble_recive_speed; 
...
static const esp_ble_mesh_client_op_pair_t vnd_op_pair[] = {
    ...
    { ESP_BLE_MESH_VENDOR_MODEL_OP_GET_TEMPERATURE, ESP_BLE_MESH_VENDOR_MODEL_OP_TEMPERATURE_STATUS },
...
static esp_ble_mesh_model_op_t vnd_op[] = {
    ...
    ESP_BLE_MESH_MODEL_OP(ESP_BLE_MESH_VENDOR_MODEL_OP_TEMPERATURE_STATUS, 2),
                                              ...
                                              esp_ble_mesh_cfg_client_cb_param_t *param)
{
    ...
    case ESP_BLE_MESH_CFG_CLIENT_SET_STATE_EVT:
        if (param->params->opcode == ESP_BLE_MESH_MODEL_OP_APP_KEY_ADD) {
            ...
            ESP_LOGW(TAG, "%s, Provision and config successfully %d", __func__,ispaired);
        }
        break;
...
void ble_mesh_send_vendor_message(uint16_t command,uint8_t VAL)
{
    ...
    opcode = ESP_BLE_MESH_VND_MODEL_OP_SEND;
        ...
        store.vnd_tid=command;
        store.custom_data=VAL;

    // Create a buffer to hold both vnd_tid and custom_data
    uint8_t message[sizeof(store.vnd_tid) + sizeof(store.custom_data)];

    // Copy vnd_tid and custom_data into the message buffer
    memcpy(message, &store.vnd_tid, sizeof(store.vnd_tid));
    memcpy(message + sizeof(store.vnd_tid), &store.custom_data, sizeof(store.custom_data));

    err = esp_ble_mesh_client_model_send_msg(vendor_client.model, &ctx, opcode,
            sizeof(message), message, MSG_TIMEOUT, true, MSG_ROLE);
    if (err != ESP_OK) {
...
void ble_mesh_GET_vendor_message()
{
    ...
    // Use the appropriate opcode to request temperature data (e.g., 0x0002)
    opcode = ESP_BLE_MESH_VENDOR_MODEL_OP_GET_TEMPERATURE;

    err = esp_ble_mesh_client_model_send_msg(vendor_client.model, &ctx, opcode,
            0, NULL, 0, true, MSG_ROLE);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to get vendor message 0x%06x", opcode);
        return;
    }
}
                                             ...
                                             esp_ble_mesh_model_cb_param_t *param)
{
    ...
    case ESP_BLE_MESH_MODEL_OPERATION_EVT:
       ...
       //    ESP_LOGI(TAG, "Recv %x, tid %d data %d, time %lldus",
        //        param->model_operation.opcode, store.vnd_tid, store.custom_data,end_time - start_time);
        }
       
         if (param->model_operation.opcode == ESP_BLE_MESH_VENDOR_MODEL_OP_TEMPERATURE_STATUS) {
            int64_t end_time = esp_timer_get_time();
                  ble_temprature = (param->model_operation.msg[1] << 8) | param->model_operation.msg[0];
                // Extract custom_data from the next byte
                ble_recive_speed = param->model_operation.msg[2];
       //     ESP_LOGI(TAG, "Recv data%x,  %d ",param->model_operation.opcode, ble_temprature);
        }
        break;
    case ESP_BLE_MESH_MODEL_SEND_COMP_EVT:
       ...
       // ESP_LOGI(TAG, "Send 0x%x", param->model_send_comp.opcode);
        break;
    ...
    case ESP_BLE_MESH_CLIENT_MODEL_SEND_TIMEOUT_EVT:
      ...
      // ble_mesh_send_vendor_message(previous_cmd,previous_value);
        break;
    default:
        break;
    }
...
{
    // static bool iswakeup;
    ESP_LOGI(TAG, "ESblutooth callbck %d",bluetoothButton.value);
    
    // if(iswakeup)
    // {
    //     iswakeup=0; 
    //     dgus_set_brightness(0);
    //     esp_deep_sleep_start();
    // }
    // else
    // {
    //     dgus_set_brightness(90);
    //     iswakeup=1;
    if (bluetoothButton.value==5) //on
    {
        dgus_set_var(0x1102,56);  // list 
