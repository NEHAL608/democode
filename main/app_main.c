...
#define CONFIG_ESPRAINMAKER_ENABLE 1
esp_rmaker_device_t *switch_device;
esp_rmaker_device_t *light_device;
esp_rmaker_device_t *fan_device,*Mode_device;
            ...
            const esp_rmaker_param_val_t val, void *priv_data, esp_rmaker_write_ctx_t *ctx)
{
    ...
    if (strcmp(param_name, ESP_RMAKER_DEF_POWER_NAME) == 0) {
        ESP_LOGI(TAG, "Received value 1= %s for %s - %s",
                val.val.b? "true" : "false", device_name, param_name);
            app_driver_set_state(val.val.b);
        
    } else if (strcmp(param_name, ESP_RMAKER_DEF_BRIGHTNESS_NAME) == 0) {
        ESP_LOGI(TAG, "Received value2 = %d for %s - %s",
                val.val.i, device_name, param_name);
    } else if (strcmp(param_name, ESP_RMAKER_DEF_SPEED_NAME) == 0) {
        ESP_LOGI(TAG, "Received value3 = %d for %s - %s",
                val.val.i, device_name, param_name);
                    app_motor_set_state(val.val.i);

    } 
      else if (strcmp(param_name, ESP_RMAKER_UI_SLIDER) == 0) {
        ESP_LOGI(TAG, "Received ESP_RMAKER_UI_SLIDER = %d for %s - %s",
                val.val.i, device_name, param_name);
                  app_mode_set_state(val.val.i);
...
void app_main()
{
    ...
    esp_rmaker_node_t *node = esp_rmaker_node_init(&rainmaker_cfg, "master-device", "Device");
    if (!node) {
   ...
   switch_device = esp_rmaker_switch_device_create("play/pause", NULL, 0);
    ...
    fan_device = esp_rmaker_fan_device_create("Motor", NULL, DEFAULT_FAN_POWER);
    ...
    esp_rmaker_device_add_param(fan_device, esp_rmaker_speed_param_create(ESP_RMAKER_DEF_SPEED_NAME, DEFAULT_FAN_SPEED));
     // Modified implementation of esp_rmaker_speed_param_create
     esp_rmaker_param_t *param = esp_rmaker_param_create(ESP_RMAKER_UI_SLIDER, ESP_RMAKER_PARAM_SPEED,
     esp_rmaker_int(1), PROP_FLAG_READ | PROP_FLAG_WRITE);
     esp_rmaker_param_add_ui_type(param, ESP_RMAKER_UI_SLIDER);
     esp_rmaker_param_add_bounds(param, esp_rmaker_int(1), esp_rmaker_int(4), esp_rmaker_int(1));
...
//    Mode_device = esp_rmaker_fan_device_create("Mode", NULL, 1);
//     esp_rmaker_device_add_cb(Mode_device, write_cb, NULL);
...
// // Add the modified param directly to the device
// esp_rmaker_device_add_param(Mode_device, param);


//     esp_rmaker_node_add_device(node, Mode_device);
    ...
    if (err != ESP_OK) {
     ...
     {
        xTaskCreate(wifi_task, "wifi_task", 1024 * 3, NULL, 2, NULL);
        Weather_Init();
